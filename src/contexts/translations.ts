// Comprehensive English → Arabic dictionary used by the global auto-translator.
// Keys are the EXACT English strings (case-sensitive) that appear in the UI.
// Add new entries here whenever new visible English text is introduced.

// Ollama translation fallback — local LLM-based machine translation
// Override via VITE_OLLAMA_URL in .env
export const OLLAMA_URL =
  (typeof import.meta !== "undefined" && import.meta.env?.VITE_OLLAMA_URL) ||
  "http://localhost:11434";

// Using qwen2.5:3b — general Qwen2.5 model, good at Arabic translation
const TRANSLATION_MODEL = "qwen2.5:3b";

// In-memory cache seeded with dictionary entries. Async translations get cached here too.
export const translationCache = new Map<string, string>();

// LibreTranslate API fallback for when Ollama is unavailable
const LIBRE_TRANSLATE_URL = "https://libretranslate.com/translate";

async function translateViaLibre(texts: string[]): Promise<Map<string, string>> {
  const results = new Map<string, string>();
  for (const text of texts) {
    try {
      const res = await fetch(LIBRE_TRANSLATE_URL, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ q: text, source: "en", target: "ar", format: "text" }),
        signal: AbortSignal.timeout(10000),
      });
      if (!res.ok) continue;
      const data = await res.json();
      const translated = data.translatedText as string;
      if (translated && translated !== text) {
        translationCache.set(text, translated);
        results.set(text, translated);
      }
    } catch {
      // Silently continue
    }
  }
  return results;
}

// Translate ALL provided English texts to Arabic in a single Ollama call.
// Falls back to LibreTranslate if Ollama is unavailable.
// Returns a Map of original → translated Arabic for every input string.
export async function fetchTranslations(texts: string[]): Promise<Map<string, string>> {
  const results = new Map<string, string>();
  const uncached = texts.filter((t) => !translationCache.has(t));

  // Return cached results immediately
  for (const t of texts) {
    const c = translationCache.get(t);
    if (c) results.set(t, c);
  }

  if (uncached.length === 0) return results;

  // Build a JSON array of all uncached texts
  const inputJson = JSON.stringify(uncached);

  const prompt = `You are an expert Arabic translator translating UI text for a web application.
Rules:
- Produce fluent, natural Modern Standard Arabic suitable for user interfaces.
- Keep translations concise and UI-friendly.
- Preserve numbers, punctuation and placeholders exactly (e.g. '%s', '{count}', '{{name}}'). Do NOT translate them.
- Do NOT translate or alter code identifiers, file names, or CSS classes.
- Return ONLY a valid JSON array of translated strings in the SAME ORDER as the input.
- Every input string MUST have a corresponding Arabic translation in the output.

Input JSON array: ${inputJson}

Output JSON array:`;

  // Try Ollama first
  let ollamaFailed = false;
  try {
    const res = await fetch(`${OLLAMA_URL}/api/generate`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        model: TRANSLATION_MODEL,
        prompt,
        stream: false,
      }),
      signal: AbortSignal.timeout(30000),
    });
    if (res.ok) {
      const data = await res.json();
      const raw = data.response as string;
      const jsonMatch = raw.match(/\[[\s\S]*\]/);
      if (jsonMatch) {
        const translations = JSON.parse(jsonMatch[0]) as string[];
        if (Array.isArray(translations)) {
          for (let i = 0; i < uncached.length && i < translations.length; i++) {
            const translated = translations[i]?.trim();
            if (translated) {
              translationCache.set(uncached[i], translated);
              results.set(uncached[i], translated);
            }
          }
          return results;
        }
      }
    }
    ollamaFailed = true;
  } catch {
    ollamaFailed = true;
  }

  // Fallback: LibreTranslate for each uncached text
  if (ollamaFailed && uncached.length > 0) {
    const libreResults = await translateViaLibre(uncached);
    for (const [k, v] of libreResults) results.set(k, v);
  }

  return results;
}

export const enToAr: Record<string, string> = {
  // Brand & shared
  "Alheb Islamic": "الحبيب الإسلامية",
  "Alheib Mixed Day & Boarding School": "مدرسة الحبيب المختلطة النهارية والداخلية",
  "School Management System - Uganda New Curriculum": "نظام إدارة المدرسة - المنهج الأوغندي الجديد",
  "For Parents": "لأولياء الأمور",
  "Parents": "أولياء الأمور",
  "For Teachers": "للمعلمين",
  "Teachers": "المعلمون",
  "Parent Portal": "بوابة ولي الأمر",
  "Welcome back! Term 3, 2024 - Alheib Mixed Day & Boarding School": "مرحبًا بعودتك! الفصل الثالث، 2024 - مدرسة الحبيب المختلطة النهارية والداخلية",
  "Welcome Back!": "مرحبًا بعودتك!",
  "What you can do here": "ما يمكنك فعله هنا",
  "Track your children's progress and stay connected with the school": "تابع تقدم أبنائك وابقَ على تواصل مع المدرسة",
  "No Children Linked": "لا يوجد أبناء مرتبطون",
  "Your account hasn't been linked to any learners yet. Please contact the school administration to link your children to your account.":
    "لم يتم ربط حسابك بأي متعلم بعد. يرجى التواصل مع إدارة المدرسة لربط أبنائك بحسابك.",
  "Last 7 Days Attendance": "حضور آخر 7 أيام",
  "View Grades": "عرض الدرجات",
  "Schedule": "الجدول",
  "School Calendar": "تقويم المدرسة",
  "program": "البرنامج المدرسي",
  "health": "الصحة والعيادة",
  "Contact Teachers": "التواصل مع المعلمين",
  "Report Cards": "بطاقات التقارير",
  "My Profile": "ملفي الشخصي",
  "Learning Roadmap": "خارطة طريق التعلم",
  "Teacher Workspace": "مساحة عمل المعلم",
  "Loading live performance metrics...": "جاري تحميل مقاييس الأداء المباشرة...",
  "Manage your classes and learner progress": "إدارة فصولك وتقدم المتعلمين",
  "Islamic Studies Progress": "تقدم الدراسات الإسلامية",
  "Quran & Hifdh Completion": "إكمال القرآن والحفظ",
  "Student Character (Akhlaaq)": "أخلاق الطلاب",
  "Class Performance Pulse": "نبض أداء الفصل",
  "Daily Schedule": "الجدول اليومي",
  "Resources & Tools": "الموارد والأدوات",
  "Lesson Plans": "خطط الدروس",
  "Academic Resources": "الموارد الأكاديمية",
  "Enter Progress": "إدخال التقدم",
  "View All Classes": "عرض جميع الفصول",
  "Term Progress": "تقدم الفصل الدراسي",
  "Syllabus Coverage": "تغطية المنهج",
  "Character Development": "تطوير الشخصية",
  "Today's Highlights": "أبرز أحداث اليوم",
  "Madrasa Module": "وحدة المدرسة",
  "Avg. Progress": "متوسط التقدم",
  "Term 3 Weekly Progress": "التقدم الأسبوعي للفصل الثالث",
  "Highest": "الأعلى",
  "Full Report": "تقرير كامل",
  "View Full Calendar": "عرض التقويم الكامل",
  "View All Notices": "عرض جميع الإشعارات",
  "Madrasa": "مدرسة إسلامية",
  "Details": "التفاصيل",
  "Assigned Students": "الطلاب المعينون",
  "Today's Attendance": "حضور اليوم",
  "Grading Status": "حالة التقييم",
  "Avg. Akhlaaq Score": "متوسط درجة الأخلاق",
  "+3 this week": "+3 هذا الأسبوع",
  "2 students absent": "طالبان غائبان",
  "12 pending marks": "12 درجة معلقة",
  "Above school avg": "أعلى من متوسط المدرسة",

  // Dashboard / Index
  "Dashboard": "لوحة التحكم",
  "Uganda New Curriculum": "المنهج الأوغندي الجديد",
  "P1-P7 Competency-Based Education System": "نظام التعليم القائم على الكفاءات من P1 إلى P7",
  "Term 3, 2024": "الفصل الثالث، 2024",
  "Week 8 of 14": "الأسبوع 8 من 14",
  "Total Learners": "إجمالي المتعلمين",
  "active": "نشط",
  "Active staff": "موظفون نشطون",
  "No teachers yet": "لا يوجد معلمون بعد",
  "Classes (P1-P7)": "الفصول (P1-P7)",
  "All active": "جميعها نشطة",
  "No classes yet": "لا توجد فصول بعد",
  "Attendance Rate": "نسبة الحضور",
  "No data today": "لا توجد بيانات اليوم",
  "from last week": "مقارنة بالأسبوع الماضي",
  "Recent Activity": "النشاط الأخير",
  "No recent activity": "لا يوجد نشاط حديث",
  "Quick Actions": "إجراءات سريعة",
  "Add Student": "إضافة طالب",
  "Generate Report": "توليد تقرير",
  "Schedule Event": "جدولة حدث",
  "Send Notice": "إرسال إشعار",
  "Class Overview": "نظرة عامة على الفصول",
  "Uganda New Curriculum Structure": "هيكل المنهج الأوغندي الجديد",
  "Classes": "الفصول",
  "Upcoming Events": "الأحداث القادمة",
  "No upcoming events scheduled": "لا توجد أحداث قادمة مجدولة",
  "Today": "اليوم",
  "Tomorrow": "غدًا",
  "Scheduled notification": "إشعار مجدول",
  "Fee Collection Summary": "ملخص تحصيل الرسوم",
  "Expected Fees": "الرسوم المتوقعة",
  "Collected": "تم التحصيل",
  "Outstanding": "المستحقات المتبقية",
  "Collection rate": "نسبة التحصيل",
  "Recent Fee Payments": "دفعات الرسوم الأخيرة",
  "No payments yet": "لا توجد دفعات بعد",
  "Students with Outstanding Balances": "طلاب لديهم أرصدة متبقية",
  "No students with outstanding balances": "لا يوجد طلاب لديهم أرصدة متبقية",

  // Sidebar / nav
  "Learners": "المتعلمون",
  "Staff & Workers": "الموظفون والعمال",
  "Marks Entry": "إدخال الدرجات",
  "Reports": "التقارير",
  "Visitors": "الزوار",
  "Attendance": "الحضور",
  "Fee Management": "إدارة الرسوم",
  "Salary Management": "إدارة الرواتب",
  "ID Cards": "البطاقات الشخصية",
  "User Management": "إدارة المستخدمين",
  "Notifications": "الإشعارات",
  "System Settings": "إعدادات النظام",
  "Staff Assignments": "تعيينات الموظفين",
  "Logout": "تسجيل الخروج",
  "Primary School": "مدرسة ابتدائية",
  "Administrator": "مدير النظام",
  "Teacher": "معلم",
  "Parent": "ولي الأمر",
  "Staff": "موظفون",
  "Audit Log": "سجل التدقيق",
  "Immutable record of every financial and structural modification": "سجل غير قابل للتعديل لكل تعديل مالي وهيكلي",
  "Finance Hub": "مركز المالية",
  "Alheib Financial Ecosystem & Procurement Control": "النظام المالي لمدرسة الحبيب ومراقبة المشتريات",
  "Accounts": "الحسابات",
  "Manage hierarchical ledger and financial organization": "إدارة دفتر الأستاذ الهرمي والتنظيم المالي",
  "Procurement": "المشتريات",
  "Manage Purchase Orders and Sequential Approvals": "إدارة أوامر الشراء والموافقات المتسلسلة",
  "Petty Cash": "الصندوق الصغير",
  "Manage box liquidity and project-specific petty cash cycles": "إدارة سيولة الصندوق ودورات الصندوق الصغير الخاصة بالمشاريع",
  "Financial & Procurement": "المالية والمشتريات",
  "Manage budget requests, procurement, and financial reports": "إدارة طلبات الميزانية والمشتريات والتقارير المالية",
  "Review and release staff expense claims": "مراجعة والإفراج عن مطالبات نفقات الموظفين",
  "Workforce & HR Hub": "مركز القوى العاملة والموارد البشرية",
  "Unified management center for Teachers, Staff, and Support Workers": "مركز إدارة موحد للمعلمين والموظفين والعمال المساندين",
  "Bank Reconciliation": "تسوية البنك",
  "Match bank statement lines to ledger entries": "مطابقة بنود كشف الحساب البنكي مع قيود دفتر الأستاذ",
  "URA / PAYE / NSSF / VAT summary printouts": "مطبوعات ملخص ضريبة الدخل والتأمين الاجتماعي وضريبة القيمة المضافة",
  "Daily Duty Form": "نموذج المناوبة اليومية",
  "Daily duty roster and register": "جدول المناوبة اليومية والسجل",
  "Director Approvals": "موافقات المدير",
  "Administration sign-off -- final stage": "اعتماد الإدارة - المرحلة النهائية",
  "Director - User Control": "المدير - التحكم في المستخدمين",
  "Create, restrict, suspend, message and kick any user in real-time": "إنشاء وتقييد وتعليق وإرسال رسائل وطرد أي مستخدم في الوقت الفعلي",
  "Discipline Management": "إدارة الانضباط",
  "Manage student conduct and discipline records": "إدارة سجلات سلوك الطلاب وانضباطهم",
  "Discount Management": "إدارة الخصومات",
  "Configure and manage fee discount rules": "تكوين وإدارة قواعد خصم الرسوم",
  "Results & Analysis": "النتائج والتحليل",
  "Deep dive into termly performance data": "تحليل متعمق لبيانات الأداء الفصلية",
  "Directing Academic Affairs, Curriculum & Standards": "توجيه الشؤون الأكاديمية والمناهج والمعايير",
  "DOS Theology": "مدير الدراسات - الشؤون الإسلامية",
  "Director of Studies -- Islamic & IPLE Division": "مدير الدراسات - قسم المواد الإسلامية والامتحان الإسلامي",
  "Exam Scheduling": "جدولة الامتحانات",
  "Academic Assessments & Invigilator Rotas": "التقييمات الأكاديمية وجداول المراقبين",
  "DOS Hostel Oversight": "إشراف مدير الدراسات على السكن",
  "Academic supervision of boarding facilities": "الإشراف الأكاديمي على مرافق الإقامة الداخلية",
  "Islamic Primary Leaving Examination": "الامتحان الإسلامي لترك المدرسة الابتدائية",
  "Lesson Plan Tracking": "تتبع خطط الدروس",
  "Review and approve teacher instructional materials": "مراجعة والموافقة على المواد التعليمية للمعلمين",
  "P7 Candidate Management": "إدارة مرشحي الصف السابع",
  "PLE Preparation & UNEB Registration Portal": "بوابة التحضير لامتحان ترك المدرسة الابتدائية والتسجيل في المجلس الوطني للامتحانات",
  "Plan your weekly teaching topics and lessons": "خطط لموضوعاتك ودروسك التعليمية الأسبوعية",
  "Configure venues, assign classes, and generate exam seating plans": "تكوين القاعات وتعيين الفصول وإنشاء خطط جلوس الامتحانات",
  "Manage subjects offered at school": "إدارة المواد الدراسية المقدمة في المدرسة",
  "Subject Load Management": "إدارة العبء الدراسي",
  "Assign subjects to teachers across classes": "تعيين المواد الدراسية للمعلمين عبر الفصول",
  "Syllabus Coverage & Academic Audit": "تغطية المنهاج والتدقيق الأكاديمي",
  "Monitor teacher commitments, actual delivery, and topic performance": "مراقبة التزامات المعلمين والتسليم الفعلي وأداء الموضوعات",
  "Syllabus Reports & Head Teacher Dashboard": "تقارير المنهاج ولوحة تحكم المعلم الرئيسي",
  "Monitor curriculum coverage across classes, subjects, and teachers": "مراقبة تغطية المنهج عبر الفصول والمواد والمعلمين",
  "Package Delivery": "توصيل الطرود",
  "Receive, log, and track packages at the gate": "استلام وتسجيل وتتبع الطرود عند البوابة",
  "Learner & Staff Movement Authorization": "تصريح حركة المتعلمين والموظفين",
  "Appointment Verification": "التحقق من المواعيد",
  "Scan or enter appointment ID to verify and check in visitors": "مسح أو إدخال معرف الموعد للتحقق من الزوار وتسجيل دخولهم",
  "Campus security and visitor management": "أمن الحرم الجامعي وإدارة الزوار",
  "Shift Handover": "تسليم المناوبة",
  "Security Accountability & Continuity": "المساءلة الأمنية واستمرارية العمل",
  "Host Verification": "التحقق من المضيف",
  "Enter the 4-digit PIN from the visitor's gate pass": "أدخل رمز PIN المكون من 4 أرقام من تصريح دخول الزائر",
  "Gate Security & Traffic Monitoring": "أمن البوابة ومراقبة حركة المرور",
  "Gate Pass Verification": "التحقق من تصريح البوابة",
  "Scan and verify printed gate passes": "مسح والتحقق من تصاريح البوابة المطبوعة",
  "Student Identity Verification": "التحقق من هوية الطالب",
  "Scan student ID card barcode at the gate": "مسح رمز البطاقة التعريفية للطالب عند البوابة",
  "Health & Medical Records": "السجلات الصحية والطبية",
  "Sick bay, clinical assessments, pharmacy, and health analytics": "المستوصف والتقييمات السريرية والصيدلية والتحليلات الصحية",
  "Hikvision DS-K1A802AMF-B Fingerprint Terminal": "جهاز البصمة Hikvision DS-K1A802AMF-B",
  "Digital Homework Portal": "بوابة الواجبات المنزلية الرقمية",
  "Upload and track learner's books scans and holiday work": "رفع وتتبع مسح كتب المتعلمين وأعمال العطلات",
  "Hostel & Welfare": "السكن والرعاية",
  "Manage dormitories, residents, and welfare items issued to learners": "إدارة المساكن والمقيمين ومواد الرعاية المقدمة للمتعلمين",
  "Secretary Hub": "مركز السكرتارية",
  "Office operations, communications & document management": "عمليات المكتب والاتصالات وإدارة المستندات",
  "Head Teacher Dashboard": "لوحة تحكم المعلم الرئيسي",
  "Academic oversight & school operations": "الإشراف الأكاديمي وعمليات المدرسة",
  "Internal Affairs Dashboard": "لوحة تحكم الشؤون الداخلية",
  "Staff oversight, discipline & internal operations": "الإشراف على الموظفين والانضباط والعمليات الداخلية",
  "Staff Workspace": "مساحة عمل الموظف",
  "Daily operations & support tasks": "العمليات اليومية والمهام المساندة",
  "Direct Manager": "المدير المباشر",
  "Cross-department oversight & approvals": "الإشراف عبر الإدارات والموافقات",
  "Center Director": "مدير المركز",
  "Strategic oversight & executive command": "الإشراف الاستراتيجي والقيادة التنفيذية",
  "Inventory Audit Trail": "مسار تدقيق المخزون",
  "Complete tracking of all school supply movements and approvals": "تتبع كامل لحركات مستلزمات المدرسة والموافقات",
  "Kitchen Inventory": "مخزون المطبخ",
  "Manage food stock & supplies": "إدارة مخزون الطعام والمستلزمات",
  "Food inventory, meal planning & store orders": "مخزون الطعام وتخطيط الوجبات وطلبات المخزن",
  "Meal Management": "إدارة الوجبات",
  "Track meals served & food consumption": "تتبع الوجبات المقدمة واستهلاك الطعام",
  "Weekly Menu Planner": "مخطط القائمة الأسبوعية",
  "Plan breakfast, lunch & dinner for the week": "خطط للإفطار والغداء والعشاء للأسبوع",
  "Food supply ordering & requests": "طلب وتقديم طلبات الإمدادات الغذائية",
  "Catalog, borrow, return, and manage library resources": "فهرسة واستعارة وإعادة وإدارة موارد المكتبة",
  "Islamic Education Suite": "مجموعة التعليم الإسلامي",
  "Quran, Salah, Akhlaaq, and Madrasa administration": "القرآن والصلاة والأخلاق وإدارة المدرسة الإسلامية",
  "Process learner arrival clearances": "معالجة تصاريح وصول المتعلمين",
  "Boarding dormitories and their residents": "المساكن الداخلية والمقيمين فيها",
  "Learner Essentials": "أساسيات المتعلم",
  "Track learner belongings & essential items": "تتبع متعلقات المتعلمين والأغراض الأساسية",
  "Matron's Dashboard": "لوحة تحكم المشرفة",
  "Boarding & Dormitory Management": "إدارة السكن الداخلي والمساكن",
  "Dormitory Residents": "مقيمو المساكن",
  "Manage learners assigned to dormitories": "إدارة المتعلمين المعينين في المساكن",
  "Hostel Supplies": "مستلزمات السكن",
  "Track and manage hostel inventory items": "تتبع وإدارة عناصر مخزون السكن",
  "Laundry & Washing": "الغسيل والتنظيف",
  "Manage washing machines and laundry loads": "إدارة آلات الغسيل وأحمال الغسيل",
  "Ministry Context": "سياق الوزارة",
  "MoES Compliance, Standards & National Regulations": "الامتثال لوزارة التربية والتعليم والمعايير واللوائح الوطنية",
  "Full inventory tracking with batches, suppliers, and reorder management": "تتبع كامل للمخزون مع الدفعات والموردين وإدارة إعادة الطلب",
  "Pharmacy Reports & Emergency Response": "تقارير الصيدلية والاستجابة للطوارئ",
  "Health analytics, dispensing trends, and emergency action center": "التحليلات الصحية واتجاهات الصرف ومركز عمل الطوارئ",
  "Full prescription management - create, dispense, and track": "إدارة كاملة للوصفات الطبية - إنشاء وصرف وتتبع",
  "Appointment Management": "إدارة المواعيد",
  "Approve, manage, and track visitor appointments": "الموافقة على مواعيد الزوار وإدارتها وتتبعها",
  "Broadcast Center": "مركز البث",
  "Send announcements, reminders & critical messages to school community": "إرسال الإعلانات والتذكيرات والرسائل الهامة لمجتمع المدرسة",
  "Document Registry": "سجل المستندات",
  "Manage School Documentation & File Archive": "إدارة وثائق المدرسة وأرشيف الملفات",
  "Front desk, documents, communications & visitor control": "مكتب الاستقبال والمستندات والاتصالات والتحكم في الزوار",
  "Print Order Center": "مركز أوامر الطباعة",
  "Upload work for printing and manage print requests": "رفع العمل للطباعة وإدارة طلبات الطباعة",
  "Comprehensive orphan care and sponsorship management": "رعاية الأيتام الشاملة وإدارة الكفالة",
  "Reports & Documents": "التقارير والمستندات",
  "Generate, preview, publish & export school documents": "إنشاء ومعاينة ونشر وتصدير وثائق المدرسة",
  "Security Console": "لوحة التحكم الأمنية",
  "Hierarchical Staff Management": "إدارة الموظفين الهرمية",
  "Assign organizational scope and permissions to users": "تعيين النطاق التنظيمي والأذونات للمستخدمين",
  "Inventory Alerts": "تنبيهات المخزون",
  "Items Requiring Immediate Replenishment": "عناصر تحتاج إلى إعادة تزويد فورية",
  "Goods Receiving": "استلام البضائع",
  "Log and Verify Incoming Deliveries": "تسجيل والتحقق من التوصيلات الواردة",
  "Store Operations": "عمليات المخزن",
  "Inventory & supplier control - Alheib School": "التحكم في المخزون والموردين - مدرسة الحبيب",
  "Supplier Management": "إدارة الموردين",
  "Vet and Manage School Vendors & Service Providers": "فحص وإدارة موردي المدرسة ومقدمي الخدمات",
  "Tailor Workshop": "ورشة الخياطة",
  "Uniform management, measurements & repairs": "إدارة الزي الموحد والقياسات والإصلاحات",
  "Create and manage detailed lesson plans": "إنشاء وإدارة خطط الدروس التفصيلية",
  "Record which lessons were taught and add observations": "تسجيل الدروس التي تم تدريسها وإضافة الملاحظات",
  "Clock-in Logs & Punctuality Analysis": "سجلات تسجيل الدخول وتحليل الالتزام بالمواعيد",
  "My Requests": "طلباتي",
  "Official Alheib leave + salary advance forms": "نماذج الإجازة الرسمية والسلفة الراتبية لمدرسة الحبيب",
  "Theology Teacher": "معلم المواد الإسلامية",
  "Islamic Studies & IPLE Division": "قسم الدراسات الإسلامية والامتحان الإسلامي",
  "Manage your profile and security credentials": "إدارة ملفك الشخصي وبيانات الأمان",
  "ID Issuing Station": "محطة إصدار البطاقات التعريفية",
  "Live credential management & production": "إدارة وإنتاج البطاقات التعريفية المباشرة",
  "My Students": "طلابي",
  "Manage learners in your assigned classes": "إدارة المتعلمين في فصولك المعينة",
  "Academic Timetable": "الجدول الأكاديمي",
  "Master Schedule & Block Planning": "الجدول الرئيسي وتخطيط الكتل",
  "Communication Hub": "مركز الاتصالات",
  "Official notices, warnings & direct communication": "الإشعارات الرسمية والتحذيرات والاتصال المباشر",
  "Gate Finance": "مالية البوابة",
  "Payslips, advances and allowances": "قسائم الرواتب والسلف والبدلات",
  "My Finance": "ماليتي",
  "Electronic Payslips & Earnings History": "قسائم الرواتب الإلكترونية وسجل الأرباح",
  "Gate Events Calendar": "تقويم أحداث البوابة",
  "Campus entry schedule": "جدول دخول الحرم الجامعي",
  "My Timetable": "جدولي",
  "Your teaching schedule & room assignments": "جدول تدريسك وتعيينات الغرف",
  "Advanced academic timetable": "الجدول الأكاديمي المتقدم",
  "Gradebook": "دفتر الدرجات",
  "Academic & Islamic studies -- Uganda New Curriculum": "الدراسات الأكاديمية والإسلامية - المنهج الجديد لأوغندا",
  "Grading Scale Configuration": "تكوين مقياس التقدير",
  "Define and manage grade boundaries for all subject types": "تحديد وإدارة حدود الدرجات لجميع أنواع المواد",
  "Academic Calendar": "التقويم الأكاديمي",
  "School Program & Yearly Itinerary": "البرنامج المدرسي والخطة السنوية",
  "Manage your classes & learners": "إدارة فصولك والمتعلمين",
  "Gate Communications": "اتصالات البوابة",
  "Gate-specific notices": "إشعارات خاصة بالبوابة",
  "Curriculum Tracking & Academic Audit": "تتبع المنهج والتدقيق الأكاديمي",
  "Assign a homeroom/form teacher to each class": "تعيين معلم فصل لكل صف",
  "Headteacher Dashboard": "لوحة تحكم المعلم الرئيسي",
  "Deputy Headteacher Dashboard": "لوحة تحكم نائب المعلم الرئيسي",
  "School management & institutional oversight": "إدارة المدرسة والإشراف المؤسسي",
  "Access Restricted": "الوصول مقيد",
  "Approvals": "الموافقات",
  "Escalated": "مرفوع",
  "View": "عرض",
  "Update": "تحديث",
  "Create": "إنشاء",
  "Export": "تصدير",
  "Import": "استيراد",
  "Active": "نشط",
  "Settings": "الإعدادات",
  "Profile": "الملف الشخصي",
  "Select": "اختيار",
  "All": "الكل",
  "None": "لا شيء",
  "Week": "الأسبوع",
  "Month": "الشهر",
  "Student": "الطالب",
  "Grade": "الدرجة",
  "Pass": "نجاح",
  "Fail": "رسوب",
  "Staff Attendance": "حضور الموظفين",
  "Subject Load": "العبء الدراسي",
  "Syllabus": "المنهاج",
  "Coverage": "التغطية",
  "Curriculum": "المنهج",
  "Exams": "الامتحانات",
  "Seating Plans": "خطط الجلوس",
  "Grading Scales": "مقاييس التقدير",
  "Analysis": "التحليل",
  "Class Teachers": "معلمو الصفوف",
  "Assignments": "المهام",
  "IPLE": "الامتحان الإسلامي للدراسة الابتدائية",
  "Security/Gatekeeper": "حارس الأمن / البوابة",
  "Gate Operations": "عمليات البوابة",
  "Security & Visitor Management": "الأمن وإدارة الزوار",
  "Security & Visitor Management - Alheib Mixed Day & Boarding School": "الأمن وإدارة الزوار - مدرسة الحبيب المختلطة النهارية والداخلية",
  "accountSettings": "إعدادات الحساب",

  // Header
  "Search students, teachers...": "ابحث عن الطلاب، المعلمين...",
  "Password": "كلمة المرور",
  "Remember me": "تذكرني",
  "Sign In": "تسجيل الدخول",
  "New to the system?": "جديد في النظام؟",
  "Admin User": "المستخدم المسؤول",

  // Common auth / form placeholders
  "Your email": "بريدك الإلكتروني",
  "Your password": "كلمة مرورك",
  "your@email.com": "بريدك@الإلكتروني.com",
  "••••••••": "••••••••",
  "Enter your full name": "أدخل اسمك الكامل",
  "Minimum 6 characters": "6 أحرف على الأقل",
  "Select your role": "اختر دورك",
  "user@alheib.org": "user@alheib.org",
  "+256 700 123 456": "+256 700 123 456",
  "Leave blank to keep current password": "اتركه فارغًا للاحتفاظ بكلمة المرور الحالية",
  "Leave blank for 1234school.com": "اتركه فارغًا لاستخدام 1234school.com",

  // Select placeholders
  "Select class": "اختر الفصل",
  "Select subject": "اختر المادة",
  "Select category": "اختر الفئة",
  "Select status": "اختر الحالة",
  "Select type": "اختر النوع",
  "Select grading type": "اختر نوع التقييم",
  "Select learner": "اختر متعلمًا",
  "Select number of terms": "اختر عدد الفصول",
  "Select active term": "اختر الفصل النشط",
  "Choose a teacher": "اختر معلمًا",
  "Choose a class to track": "اختر فصلًا للمتابعة",
  "All classes": "جميع الفصول",
  "All subjects": "جميع المواد",
  "Filter by class...": "تصفية حسب الفصل...",
  "Select learner...": "اختر متعلمًا...",
  "Select candidate...": "اختر مرشحًا...",
  "Select subject...": "اختر مادة...",

  // Search placeholders
  "Search...": "بحث...",
  "Search subjects...": "ابحث عن مواد...",
  "Search by name, email, role…": "ابحث بالاسم أو البريد الإلكتروني أو الدور...",
  "Search teacher or title...": "ابحث عن معلم أو عنوان...",
  "Search purpose…": "ابحث عن الغرض...",
  "Type name or ADM...": "اكتب الاسم أو رقم القبول...",
  "Search witness name...": "ابحث عن اسم الشاهد...",
  "Search victim name...": "ابحث عن اسم الضحية...",
  "Search inventory...": "ابحث في المخزون...",
  "Search learners...": "ابحث عن متعلمين...",
  "Search users...": "ابحث عن مستخدمين...",
  "Search staff...": "ابحث عن موظف...",

  // Field labels as placeholders
  "Full name": "الاسم الكامل",
  "Explain your appeal (minimum 10 characters)...": "اشرح سبب استئنافك (10 أحرف على الأقل)...",
  "Optional notes regarding this entry...": "ملاحظات اختيارية حول هذا الإدخال...",
  "Provide details of what happened...": "قدم تفاصيل عما حدث...",
  "Provide guidance or reasons for approval/rejection...": "قدم توجيهًا أو أسباب الموافقة/الرفض...",
  "Scan student ID QR code or enter admission number": "امسح رمز QR لبطاقة الطالب أو أدخل رقم القبول",
  "Scan badge or search visitor...": "امسح الشارة أو ابحث عن زائر...",

  // Common "e.g." hints
  "e.g. MATH": "مثال: MATH",
  "e.g. Mathematics": "مثال: الرياضيات",
  "e.g. Term I Opening Ceremony": "مثال: حفل افتتاح الفصل الأول",
  "e.g. Addition of Fractions": "مثال: جمع الكسور",
  "e.g. Fuel for school van": "مثال: وقود لحافلة المدرسة",
  "e.g. Bullying": "مثال: تنمر",
  "e.g. Discuss learner progress": "مثال: مناقشة تقدم المتعلم",
  "e.g. Bachelor of Education, Diploma": "مثال: بكالوريوس تربية، دبلوم",
  "e.g. 5 Years": "مثال: 5 سنوات",
  "e.g. 1995": "مثال: 1995",
  "e.g., UMSC-001": "مثال: UMSC-001",
  "e.g., IPLE-2026-001": "مثال: IPLE-2026-001",

  // Other utils
  "Reason shown to user…": "السبب المعروض للمستخدم...",
  "Your message…": "رسالتك...",
  "Share your experience...": "شارك تجربتك...",
  "Comma-separated sub-topics": "مواضيع فرعية مفصولة بفواصل",
  "Here": "هنا",
  "Title": "العنوان",
  "Message": "الرسالة",

  // Common verbs / labels
  "Add": "إضافة",
  "Edit": "تعديل",
  "Delete": "حذف",
  "Save": "حفظ",
  "Cancel": "إلغاء",
  "Search": "بحث",
  "Filter": "تصفية",
  "Filter by role": "تصفية حسب الدور",
  "All Roles": "جميع الأدوار",
  "Print": "طباعة",
  "Download": "تحميل",
  "Generate": "توليد",
  "Upload": "رفع",
  "Submit": "إرسال",
  "Close": "إغلاق",
  "Open": "فتح",
  "Loading...": "جاري التحميل...",
  "Loading…": "جاري التحميل…",
  "No data found": "لا توجد بيانات",
  "Success": "تم بنجاح",
  "Error": "خطأ",
  "Previous": "السابق",
  "Next": "التالي",
  "Showing": "عرض",
  "of": "من",
  "learners": "متعلم",
  "Name": "الاسم",
  "Full Name": "الاسم الكامل",
  "Email": "البريد الإلكتروني",
  "Phone": "الهاتف",
  "Address": "العنوان",
  "Role": "الدور",
  "Class": "الفصل",
  "Gender": "الجنس",
  "Male": "ذكر",
  "Female": "أنثى",
  "male": "ذكر",
  "female": "أنثى",
  "Date of Birth": "تاريخ الميلاد",
  "Admission Number": "رقم القبول",
  "Guardian": "ولي الأمر",
  "Status": "الحالة",
  "Actions": "الإجراءات",
  "Contact": "جهة الاتصال",
  "District": "المنطقة",
  "Religion": "الديانة",
  "Qualification": "المؤهل",
  "Joined": "تاريخ الانضمام",
  "View Profile": "عرض الملف الشخصي",
  "View Details": "عرض التفاصيل",
  "Manage": "إدارة",
  "Unassigned": "غير معيّن",

  // Students
  "Manage learner records - Uganda New Curriculum": "إدارة سجلات المتعلمين - المنهج الأوغندي الجديد",
  "Current Term:": "الفصل الحالي:",
  "Total:": "الإجمالي:",
  "Register": "تسجيل",
  "Failed to load learners. Please try again.": "تعذّر تحميل المتعلمين. يرجى المحاولة مرة أخرى.",
  "No learners found": "لا يوجد متعلمون",
  "Register your first learner to get started": "سجّل أول متعلم للبدء",
  "Learner": "متعلم",

  // Teachers
  "Manage teaching staff - Uganda New Curriculum": "إدارة الكادر التدريسي - المنهج الأوغندي الجديد",
  "teachers": "معلمون",
  "NCDC Certified Educators": "معلمون معتمدون من NCDC",
  "Add Teacher": "إضافة معلم",
  "Failed to load teachers. Please try again.": "تعذّر تحميل المعلمين. يرجى المحاولة مرة أخرى.",
  "No teachers found": "لا يوجد معلمون",
  "Add your first teacher to get started": "أضِف أول معلم للبدء",
  "Class Teacher": "معلم الفصل",
  "No class assigned": "لم يتم تعيين فصل",
  "Since": "منذ",

  // Classes
  "Uganda New Curriculum - P1 to P7 Structure": "المنهج الأوغندي الجديد - الهيكل من P1 إلى P7",
  "active classes": "فصول نشطة",
  "Term 3, 2024 Academic Year": "السنة الدراسية الفصل الثالث 2024",
  "Add Class": "إضافة فصل",
  "Failed to load classes. Please try again.": "تعذّر تحميل الفصول. يرجى المحاولة مرة أخرى.",
  "No classes found": "لا توجد فصول",
  "Create your first class to get started": "أنشئ أول فصل للبدء",
  "No teacher assigned": "لم يتم تعيين معلم",
  "Level:": "المستوى:",

  // Staff
  "Manage non-teaching staff and workers": "إدارة الموظفين والعمال غير التدريسيين",
  "Add Staff": "إضافة موظف",
  "Failed to load staff. Please try again.": "تعذّر تحميل الموظفين. يرجى المحاولة مرة أخرى.",
  "No staff members found": "لا يوجد موظفون",
  "Add your first staff member to get started": "أضِف أول موظف للبدء",
  "staff": "موظفون",

  // Attendance
  "Track daily learner attendance - Term 3, 2024": "تتبع حضور المتعلمين اليومي - الفصل الثالث، 2024",
  "No classes available": "لا توجد فصول متاحة",
  "Save Attendance": "حفظ الحضور",
  "Total": "الإجمالي",
  "Present": "حاضر",
  "Absent": "غائب",
  "Late": "متأخر",
  "Excused": "معذور",
  "Select a class": "اختر فصلًا",
  "Select a class to take attendance": "اختر فصلًا لتسجيل الحضور",
  "No learners in this class": "لا يوجد متعلمون في هذا الفصل",
  "Checked in:": "تم التسجيل:",
  "Not recorded": "غير مسجَّل",
  "- Attendance": "- الحضور",
  "You have unsaved changes. Click \"Save Attendance\" to save.": "لديك تغييرات غير محفوظة. اضغط \"حفظ الحضور\" للحفظ.",

  // Fees
  "Manage fee structures and student payments": "إدارة هياكل الرسوم ودفعات الطلاب",
  "Total Collected": "إجمالي المُحصَّل",
  "Fee Structures": "هياكل الرسوم",
  "Collect Payment": "تحصيل دفعة",
  "History": "السجل",
  "Student Balances": "أرصدة الطلاب",
  "Bursar Rules": "قواعد أمين الصندوق",

  // Notifications
  "In-app inbox, broadcasts, templates and history": "صندوق الوارد داخل التطبيق، البث، القوالب والسجل",
  "Inbox": "صندوق الوارد",
  "Broadcast": "بث",
  "Templates": "القوالب",
  "SMS/WA Log": "سجل الرسائل القصيرة/الواتساب",
  "Your Inbox": "صندوق الوارد",
  "message(s)": "رسالة",
  "You're all caught up": "لقد قرأت كل شيء",
  "Broadcast In-App Notification": "بث إشعار داخل التطبيق",
  "Sends instantly to selected audience's inbox": "يرسل فورًا إلى صندوق وارد الجمهور المحدد",
  "Audience": "الجمهور",
  "All users": "كل المستخدمين",
  "Admins only": "المسؤولون فقط",
  "Teachers only": "المعلمون فقط",
  "Staff only": "الموظفون فقط",
  "Type": "النوع",
  "Info": "معلومة",
  "Warning": "تحذير",
  "Alert": "تنبيه",
  "Title *": "العنوان *",
  "Message *": "الرسالة *",
  "Link (optional)": "رابط (اختياري)",
  "Send Now": "إرسال الآن",
  "Message Templates": "قوالب الرسائل",
  "Pre-defined SMS/WhatsApp templates (used when external messaging is enabled)":
    "قوالب SMS/واتساب المعرفة مسبقًا (تُستخدم عند تفعيل المراسلة الخارجية)",
  "No templates yet": "لا توجد قوالب بعد",
  "Inactive": "غير نشط",
  "SMS / WhatsApp pending integration": "رسائل SMS / واتساب قيد التكامل",
  "External SMS/WhatsApp dispatch requires Twilio or WhatsApp credentials. In-app notifications are fully active above.":
    "يتطلب الإرسال الخارجي SMS/واتساب بيانات اعتماد Twilio أو واتساب. الإشعارات داخل التطبيق نشطة أعلاه.",
  "External Message Log": "سجل الرسائل الخارجية",
  "No external messages yet": "لا توجد رسائل خارجية بعد",
  "Recipient": "المستلم",
  "Channel": "القناة",
  "Date": "التاريخ",
  "Delivered": "تم التسليم",
  "Sent": "أرسلت",
  "Failed": "فشلت",
  "Pending": "معلَّق",

  // Schedule
  "Appointments, events and scheduled notifications": "المواعيد والأحداث والإشعارات المجدولة",
  "Appointments": "المواعيد",
  "Calendar": "التقويم",
  "Scheduled Notifications": "الإشعارات المجدولة",
  "Manage visitor appointments scheduled by reception": "إدارة مواعيد الزوار المجدولة من الاستقبال",
  "Upcoming": "القادمة",
  "Upcoming Sessions": "الجلسات القادمة",
  "Scheduled": "مجدولة",
  "Completed": "مكتملة",
  "All Appointments": "كل المواعيد",
  "From": "من",
  "onward": "وما بعد",
  "No appointments scheduled": "لا توجد مواعيد مجدولة",
  "Date & Time": "التاريخ والوقت",
  "Visitor": "الزائر",
  "Purpose": "الغرض",
  "Host": "المستضيف",
  "Location": "الموقع",
  "Nothing scheduled": "لا شيء مجدول",
  "Broadcasts queued to be sent later": "بث في انتظار الإرسال لاحقًا",
  "No scheduled notifications": "لا توجد إشعارات مجدولة",
  "New Appointment": "موعد جديد",
  "Edit Appointment": "تعديل الموعد",
  "Schedule Appointment": "جدولة موعد",
  "For visitors meeting staff or about a learner": "للزوار الذين يقابلون الموظفين أو لشأن متعلم",
  "Visitor Name *": "اسم الزائر *",
  "Visitor Phone": "هاتف الزائر",
  "Date *": "التاريخ *",
  "Time *": "الوقت *",
  "Duration (min)": "المدة (دقيقة)",
  "Pick": "اختر",
  "Location / Room": "الموقع / الغرفة",
  "e.g. Reception, Room 5": "مثال: الاستقبال، غرفة 5",
  "Host Staff": "المستضيف من الموظفين",

  // Visitors
  "Gate check-in / check-out and visitor records": "تسجيل الدخول/الخروج عند البوابة وسجلات الزوار",
  "Gate Desk": "مكتب البوابة",
  "On-Site": "في الموقع",
  "Re-entry Slips": "تصاريح إعادة الدخول",
  "Visit Log": "سجل الزيارات",
  "Recurring Visitors": "الزوار المتكررون",
  "Currently on-site": "موجودون حاليًا",
  "Today's appointments": "مواعيد اليوم",
  "Active re-entry slips": "تصاريح إعادة دخول نشطة",
  "Recurring visitors": "زوار متكررون",
  "Today's Scheduled Appointments": "مواعيد اليوم المجدولة",
  "One-click check-in for booked visitors": "تسجيل دخول بضغطة واحدة للزوار المحجوزين",
  "Issue Re-entry Slip": "إصدار تصريح إعادة دخول",
  "Walk-in Check-In": "تسجيل دخول مباشر",
  "No more scheduled appointments today": "لا توجد مواعيد مجدولة أخرى اليوم",
  "Visitors Currently On-Site": "الزوار الموجودون حاليًا",
  "No visitors on-site": "لا يوجد زوار في الموقع",
  "Badge": "الشارة",
  "Checked In": "تم تسجيل الدخول",
  "Action": "الإجراء",
  "Emergency Re-entry Slips": "تصاريح إعادة الدخول الطارئة",
  "Time-limited thermal passes for visitors returning after checkout": "تصاريح حرارية محدودة بزمن للزوار العائدين بعد الخروج",
  "Issue New Slip": "إصدار تصريح جديد",
  "No re-entry slips issued yet": "لم يتم إصدار تصاريح إعادة دخول بعد",
  "Serial": "الرقم التسلسلي",
  "Issued": "تاريخ الإصدار",
  "Expires": "ينتهي في",
  "Width": "العرض",

  // Reports page
  "Generate, preview, publish & lock learner reports": "توليد ومعاينة ونشر وقفل تقارير المتعلمين",
  "Year": "السنة",
  "selected": "محدد",
  "PDF": "PDF",
  "PNG ZIP": "PNG ZIP",
  "Publish & Lock": "نشر وقفل",
  "Publish whole class": "نشر الفصل بأكمله",
  "Unlock": "فتح القفل",
  "— Learners": "— المتعلمون",
  "Select all": "تحديد الكل",
  "Clear": "مسح",
  "No learners in this class.": "لا يوجد متعلمون في هذا الفصل.",

  // ID Cards
  "Generate ID cards for staff and students": "إنشاء بطاقات الهوية للموظفين والطلاب",
  "Staff ID Cards": "بطاقات هوية الموظفين",
  "Student ID Cards": "بطاقات هوية الطلاب",
  "Visitor IDs": "بطاقات هوية الزوار",
  "Export PNG": "تصدير PNG",
  "Front side only": "الوجه الأمامي فقط",
  "Back side only": "الوجه الخلفي فقط",
  "Both sides": "كلا الوجهين",

  // Settings
  "ID Card Signatures & Branding": "توقيعات وعلامة بطاقة الهوية",
  "School Logo / Crest": "شعار المدرسة",
  "No logo": "لا يوجد شعار",
  "Upload Logo": "رفع الشعار",
  "Or paste URL https://...": "أو ألصق رابط https://...",
  "Official School Stamp": "ختم المدرسة الرسمي",
  "No stamp": "لا يوجد ختم",
  "Upload Stamp": "رفع الختم",
  "Print Size Controls": "ضبط حجم الطباعة",
  "All assets auto-fit without cropping. Adjust the on-page display size below.":
    "تتكيّف جميع العناصر تلقائيًا دون اقتطاع. اضبط الحجم المعروض على الصفحة أدناه.",
  "Logo size on Report Card (px)": "حجم الشعار على بطاقة التقرير (بكسل)",
  "Logo size on ID Card (px)": "حجم الشعار على بطاقة الهوية (بكسل)",
  "Headteacher signature on Report Card (height px)": "توقيع ناظر المدرسة على بطاقة التقرير (ارتفاع بكسل)",
  "Signatures on ID Card (height px)": "التوقيعات على بطاقة الهوية (ارتفاع بكسل)",
  "Official stamp on Report Card (height px)": "الختم الرسمي على بطاقة التقرير (ارتفاع بكسل)",

  // Salary
  "Manage staff salaries": "إدارة رواتب الموظفين",
  "Payroll": "الرواتب",
  "Paid": "المدفوع",
  "Payments": "المدفوعات",
  "Records": "السجلات",
  "Add Salary": "إضافة راتب",
  "Add Salary Record": "إضافة سجل راتب",
  "Select staff": "اختر موظف",
  "Effective From": "تاريخ السريان",
  "Notes": "ملاحظات",
  "Record Payment": "تسجيل دفعة",
  "Record New Payment": "تسجيل دفعة جديدة",
  "Amount": "المبلغ",
  "Reference Number": "رقم المرجع",
  "Basic Salary": "الراتب الأساسي",
  "Allowances": "البدلات",
  "Deductions": "الخصومات",
  "Net Salary": "صافي الراتب",
  "Payment Date": "تاريخ الدفع",
  "Payment Method": "طريقة الدفع",
  "Payment History": "سجل المدفوعات",
  "Bank Transfer": "تحويل بنكي",
  "Cash": "نقدي",
  "Cheque": "شيك",
  "Mobile Payment": "دفع عبر الهاتف",

  // Marks Entry
  "Term 1": "الفصل الأول",
  "Term 2": "الفصل الثاني",
  "Term 3": "الفصل الثالث",
  "Academic": "أكاديمي",
  "Islamic": "إسلامي",
  "Save all": "حفظ الكل",
  "Copy previous term": "نسخ الفصل السابق",
  "Score": "الدرجة",
  "Subject": "المادة",
  "Term": "الفصل الدراسي",
  "Average": "المعدل",
  "Position": "الترتيب",
  "Saved": "تم الحفظ",
  "Nothing to save": "لا شيء للحفظ",
  "Enter marks first.": "أدخل الدرجات أولًا.",
  "Save failed": "فشل الحفظ",
  "Invalid mark": "درجة غير صالحة",
  "Score must be between 0 and 100": "يجب أن تكون الدرجة بين 0 و100",
  "Class Assignments": "تعيينات الفصول",
  "Academic Resource Allocation & Teacher Load": "تخصيص الموارد الأكاديمية وحمل المعلمين",
  "Add Subject": "إضافة مادة",
  "New Assignment": "تعيين جديد",
  "Assign Teacher to Class": "تعيين معلم إلى فصل",
  "Link a teacher to a specific subject in a class": "ربط معلم بمادة معينة في فصل",
  "Choose subject": "اختر مادة",
  "Assigning...": "جاري التعيين...",
  "Confirm Assignment": "تأكيد التعيين",
  "Add New Subject": "إضافة مادة جديدة",
  "Register a subject so it can be assigned to classes and lesson plans.": "سجل مادة حتى يمكن تعيينها للفصول وخطط الدروس.",
  "Subject name": "اسم المادة",
  "Save Subject": "حفظ المادة",
  "Subject name is required": "اسم المادة مطلوب",
  "Subject added successfully": "تمت إضافة المادة بنجاح",
  "New Teaching Assignment": "تعيين تدريس جديد",
  "Your Active Teaching Assignments": "تعييناتك النشطة للتدريس",
  "No active subject assignments found. Assign subjects in the DOS / Academic Assignments section to make lesson planning meaningful.": "لم يتم العثور على أي تعيينات مواضيع نشطة. قم بتعيين المواد في قسم DOS / التعيينات الأكاديمية لجعل تخطيط الدروس ذا معنى.",
  "Loading plans...": "جاري تحميل الخطط...",
  "You haven't created any lesson plans yet.": "لم تقم بإنشاء أي خطط دروس بعد.",
  "No objectives defined": "لم يتم تحديد أي أهداف",
  "Feedback:": "التعليقات:",
  "New Lesson Plan": "خطة درس جديدة",
  "Draft your instructional goals": "صمم أهدافك التعليمية",
  "Prepare and manage your weekly instructional activities": "حضِّر وأدر أنشطتك الأسبوعية التعليمية",
  "Submit Plan": "إرسال الخطة",
  "Subject & Syllabus Workflow": "سير عمل المواد والمنهج",
  "Add subjects, assign teachers and keep syllabus coverage updated so the lesson planner becomes fully meaningful for teaching staff.": "أضف المواد، وعين المعلمين وحافظ على تحديث تغطية المنهج حتى تصبح خطة الدروس ذات معنى كامل للكادر التعليمي.",
  "Manage Syllabus": "إدارة المنهج",
  "Back to Teacher Assignments": "العودة إلى تعيينات المعلمين",
  "Syllabus & Lesson Linkage": "ربط المنهج وخطة الدروس",
  "Keep subjects assigned and syllabus topics updated so teacher planners reflect the actual teaching load.": "حافظ على تعيين المواد وتحديث موضوعات المنهج حتى تعكس مخططات المعلمين الحمل الفعلي للتدريس.",
  "Sequence": "التسلسل",
  "Planned duration": "المدة المخططة",
  "weeks": "أسابيع",
  "Topic Covered": "الموضوع مغطى",
  "Mark as Covered": "وضع علامة كمكتمل",
  "Download Template": "تحميل القالب",
  "No Class Selected": "لم يتم اختيار فصل",
  "Select a class above to view and track syllabus coverage.": "اختر فصلًا أعلاه لعرض وتتبع تغطية المنهج.",
  "Loading curriculum plan...": "جاري تحميل خطة المنهج...",
  "No Plan Found": "لم يتم العثور على خطة",
  "Curriculum plans haven't been uploaded for this class yet.": "لم يتم تحميل خطط المنهج لهذا الفصل بعد.",

  // User Management
  "Manage system users and access roles": "إدارة مستخدمي النظام وأدوار الوصول",
  "Create Staff Account": "إنشاء حساب موظف",
  "Create Staff/Admin Account": "إنشاء حساب موظف / مسؤول",
  "Create a new account for teachers, staff, or administrators": "أنشئ حسابًا جديدًا للمعلمين أو الموظفين أو المسؤولين",
  "Phone *": "الهاتف *",
  "Email *": "البريد الإلكتروني *",
  "Full Name *": "الاسم الكامل *",
  "Password *": "كلمة المرور *",
  "Role *": "الدور *",
  "Min 6 characters": "6 أحرف على الأقل",
  "Create Account": "إنشاء الحساب",
  "User": "المستخدم",
  "Linked Learners": "المتعلمون المرتبطون",
  "No learners linked": "لا يوجد متعلمون مرتبطون",
  "Link Learner": "ربط متعلم",
  "Link Parent to Learner": "ربط ولي الأمر بالمتعلم",
  "Select a learner to link to this parent account": "اختر متعلمًا لربطه بحساب ولي الأمر هذا",
  "Administrators": "المسؤولون",
  "Staffs": "موظفون",

  // Misc
  "Delete this appointment?": "حذف هذا الموعد؟",
  "Appointment scheduled": "تم جدولة الموعد",
  "Appointment updated": "تم تحديث الموعد",
  "Missing fields": "حقول مفقودة",
  "Visitor name and purpose are required": "اسم الزائر والغرض مطلوبان",
  "Cancelled": "ملغى",
  "No-show": "لم يحضر",

  // Status badges (lowercase)
  "inactive": "غير نشط",
  "completed": "مكتمل",
  "pending": "معلَّق",
  "draft": "مسودة",
  "published": "منشور",
  "locked": "مقفل",
  
  // Inventory & Assets
  "Inventory & Assets": "المخزون والأصول",
  "Inventory & Stock": "المخزون والعهدة",
  "Track school supplies, equipment and fixed assets": "تتبع الموارد المدرسية والمعدات والأصول الثابتة",
  "Total Items": "إجمالي الأصناف",
  "Low Stock": "مخزون منخفض",
  "Total Assets": "إجمالي الأصول",
  "Categories": "الفئات",
  "Stock Management": "إدارة المخزون",
  "Fixed Assets": "الأصول الثابتة",
  "Issuance Log": "سجل الصرف",
  "New Item": "صنف جديد",
  "Restock": "تزويد المخزون",
  "Item Name": "اسم الصنف",
  "Category": "الفئة",
  "Stock Level": "مستوى المخزون",
  "Unit": "الوحدة",
  "In Stock": "متوفر",
  "Issue": "صرف / إعطاء",
  "Add Inventory Item": "إضافة صنف للمخزون",
  "Create a new item in the school inventory": "إنشاء صنف جديد في مخزون المدرسة",
  "Item Name *": "اسم الصنف *",
  "Category *": "الفئة *",
  "Unit *": "الوحدة *",
  "Min Stock Alert": "تنبيه الحد الأدنى",
  "Initial Stock": "المخزون الابتدائي",
  "Description": "الوصف",
  "Create Item": "إنشاء الصنف",
  "Record an item being given to a student or staff member": "تسجيل صنف تم إعطاؤه لطالب أو موظف",
  "Recipient Type": "نوع المستلم",
  "Student / Learner": "طالب / متعلم",
  "Staff Member": "موظف",
  "General / Office Use": "استخدام عام / مكتب",
  "Select Learner": "اختر المتعلم",
  "Select Staff": "اختر الموظف",
  "Remarks / Purpose": "ملاحظات / الغرض",
  "Quantity *": "الكمية *",
  "Quantity to Add *": "الكمية المضافة *",
  "Add Stock": "إضافة للمخزون",

  // Inventory Workflow
  "Pending Approvals": "طلبات في انتظار الموافقة",
  "Bulk Issue": "صرف بالجملة",
  "Bulk Issue Items": "صرف أصناف بالجملة",
  "Distribute items to multiple students at once. Admin approval required.": "توزيع الأصناف على طلاب متعددين في وقت واحد. يتطلب موافقة المسؤول.",
  "Select Pupils": "اختر الطلاب",
  "Qty Per Pupil": "الكمية لكل طالب",
  "Custodian": "المسؤول عن العهدة",
  "Person in Charge": "الشخص المسؤول",
  "Tracking Number": "رقم التتبع",
  "Status ": "الحالة",
  "Approve": "موافقة",
  "Reject": "رفض",
  "Gate Pass": "تصريح بوابة",
  "Gate Pass Issued": "تم إصدار تصريح البوابة",
  "This receipt must be presented at the gate for confirmation.": "يجب تقديم هذا الإيصال عند البوابة للتأكيد.",
  "Verified by Admin": "معتمد من الإدارة",
  "Print Pass": "طباعة التصريح",
  "Wait for Approval": "في انتظار الموافقة",
  "Dispatched": "تم الإرسال / الخروج",
  "Tracking Number *": "رقم التتبع *",
  "On-Site Visitors": "الزوار في الموقع",
  "Today's Appts": "مواعيد اليوم",
  "Active Re-entry": "إعادة دخول نشطة",
  "Items Out (Pending)": "الأصناف الخارجة (قيد الانتظار)",
  "Currently On-Site": "موجودون حاليًا",
  "Verified": "تم التحقق",
  "Asset Clearance": "تخليص الأصول",
  "Security Dashboard": "لوحة تحكم الأمن",

  // Gate & Visitor
  "Gate Control Center": "مركز التحكم في البوابة",
  "Physical security and visitor access management": "الأمن المادي وإدارة دخول الزوار",
  "Check In Visitor": "تسجيل دخول زائر",
  "From scheduled appointment": "من موعد مجدول",
  "Walk-in visitor": "زائر بدون موعد",
  "Recurring visitor": "زائر متكرر",
  "Recurring visitor (vendor, contractor, etc.)": "زائر متكرر (مورد، مقاول، إلخ)",
  "Reusable record for recurring visitors": "سجل قابل لإعادة الاستخدام للزوار المتكررين",
  "Scan National ID": "مسح بطاقة الهوية الوطنية",
  "Scan Uganda National ID": "مسح بطاقة الهوية الوطنية الأوغندية",
  "Scan ID barcode...": "امسح الباركود...",
  "or type manually": "أو اكتب يدويًا",
  "Visitor saved": "تم حفظ الزائر",
  "Visitor checked in": "تم تسجيل دخول الزائر",
  "Day pass issued": "تم إصدار تصريح اليوم",
  "Name required": "الاسم مطلوب",
  "Visitor name required": "اسم الزائر مطلوب",
  "Add Visitor": "إضافة زائر",
  "New Visitor": "زائر جديد",
  "Student Gate Scan": "مسح بوابة الطالب",
  "Learner Identity Verification": "التحقق من هوية المتعلم",
  "Scan the student barcode or ID card to verify access": "امسح الباركود أو بطاقة هوية الطالب للتحقق من الوصول",
  "Scan student barcode...": "امسح باركود الطالب...",
  "Verified Student": "تم التحقق من الطالب",
  "View All": "عرض الكل",
  "No incidents reported recently.": "لا توجد حوادث مسجلة مؤخرًا.",
  "Infirmary Visits": "زيارات العيادة",
  "On Medication": "تحت العلاج",
  "Pending Alerts": "تنبيهات معلقة",
  "Recent Incidents": "الحوادث الأخيرة",
  "Latest medical occurrences reported": "آخر الحوادث الطبية المسجلة",
  "Active Medications": "الأدوية النشطة",
  "Most recently dispensed medication": "آخر دواء تم صرفه",
  "No medication dispensed yet today.": "لم يتم صرف أي دواء اليوم حتى الآن.",
  "Manage Medications": "إدارة الأدوية",
  "Medical Center": "المركز الطبي",
  "School Infirmary & Health Management": "العيادة المدرسية وإدارة الصحة",
  "Gate Security — Barcode Scanner": "أمن البوابة — ماسح الباركود",
  "Point scanner at ID barcode": "وجه الماسح نحو باركود الهوية",
  "Use the handheld barcode scanner on the PDF417 barcode on the back of the Uganda National ID": "استخدم ماسح الباركود المحمول على باركود PDF417 الموجود خلف بطاقة الهوية الوطنية الأوغندية",
  "Scanner output appears here...": "يظهر ناتج المسح هنا...",
  "Confirm Identity": "تأكيد الهوية",
  "Rescan": "إعادة مسح",
  "Scanned Identity": "الهوية الممسوحة",
  "National ID Verified": "تم التحقق من الهوية الوطنية",
  "Barcode Scanned": "تم مسح الباركود",
  "Clinic Management": "إدارة العيادة",
  "School Sick Bay & Student Health Log": "العيادة المدرسية وسجل صحة الطلاب",
  "Health Incidents": "الحوادث الصحية",
  "Record and Track Medical Occurrences": "تسجيل وتتبع الحوادث الطبية",
  "Log Visit": "تسجيل زيارة",
  "Dispense Medicine": "صرف دواء",
  "Medication Register": "سجل الأدوية",
  "Dispensing & Pharmacy Control": "الصرف والتحكم في الصيدلية",
  "New Visit": "زيارة جديدة",
  "Log Trauma": "تسجيل حادث",
  "Health Screen": "فحص صحي",
  "Immunization": "تطعيم",
  "Open Infirmary Hours": "ساعات عمل العيادة",
  "Morning Surgery": "العيادة الصباحية",
  "Emergency Cover": "تغطية الطوارئ",
  "24 Hours": "24 ساعة",
  "Medication Review": "مراجعة الأدوية",
  "Recently dispensed medication": "آخر الأدوية المصروفة",
  "Select District": "اختر المنطقة",
  "Select Sub-county": "اختر المقاطعة الفرعية",
  "Select Parish": "اختر الرعية",
  "Select Village": "اختر القرية",
  "Street / Landmark": "الشارع / المَعلم",
  "Select District first": "اختر المنطقة أولاً",
  "Select Sub-county first": "اختر المقاطعة الفرعية أولاً",
  "Select Parish first": "اختر الرعية أولاً",

  // Auth & Login Page
  "Sign in to access the school management system": "سجّل دخولك للوصول إلى نظام إدارة المدرسة",
  "accounts are created when you register your child at the school office": "يتم إنشاء الحسابات عند تسجيل طفلك في مكتب المدرسة",
  "accounts are created by administration": "يتم إنشاء الحسابات من قبل الإدارة",
  "For Administration": "للإدارة",
  "Track your child's attendance, grades, and communicate with teachers": "تابع حضور طفلك ودرجاته والتواصل مع المعلمين",
  "Manage classes, take attendance, and record learner progress": "إدارة الفصول وتسجيل الحضور وتسجيل تقدم المتعلمين",
  "Full access to manage all school operations and staff": "الوصول الكامل لإدارة جميع عمليات المدرسة والموظفين",
  "Account Settings": "إعدادات الحساب",
  "Accountant Dashboard": "لوحة تحكم المحاسب",
  "Active Learners": "المتعلمون النشطون",
  "Add Book": "إضافة كتاب",
  "Add Dormitory": "إضافة مهجع",
  "Add Fee": "إضافة رسوم",
  "Add Fee Structure": "إضافة هيكل رسوم",
  "Add learners to this class first.": "أضف المتعلمين إلى هذا الفصل أولاً.",
  "Add New Class": "إضافة فصل جديد",
  "All Classes": "جميع الفصول",
  "All Learners": "جميع المتعلمين",
  "Attendance %": "نسبة الحضور",
  "Attendance Today": "حضور اليوم",
  "Attendance saved successfully": "تم حفظ الحضور بنجاح",
  "Attendance updated": "تم تحديث الحضور",
  "Board Members": "أعضاء المجلس",
  "Board members list is not initialized.": "قائمة أعضاء المجلس غير مهيأة.",
  "Behind Schedule": "متأخر عن الجدول",
  "Class Schedule": "الجدول الدراسي",
  "Class Level": "مستوى الفصل",
  "Confirm Attendance": "تأكيد الحضور",
  "Constitution": "الدستور",
  "Create Class": "إنشاء فصل",
  "Cross-team performance pulse": "مؤشر أداء الفرق المتعددة",
  "Delete All": "حذف الكل",
  "Department KPIs": "مؤشرات أداء القسم",
  "Deputy Head Teacher": "نائب مدير المدرسة",
  "Director": "المدير",
  "Director Dashboard": "لوحة تحكم المدير",
  "Document Details": "تفاصيل الوثيقة",
  "Edit Book": "تعديل الكتاب",
  "Edit Subject": "تعديل المادة",
  "Edit Template": "تعديل القالب",
  "Email is required": "البريد الإلكتروني مطلوب",
  "Email or Phone": "البريد الإلكتروني أو الهاتف",
  "Error:": "خطأ:",
  "Executive Reports": "التقارير التنفيذية",
  "Export by class": "تصدير حسب الفصل",
  "Failed to add teacher": "فشل إضافة المعلم",
  "Failed to cancel": "فشل الإلغاء",
  "Failed to create": "فشل الإنشاء",
  "Fetching staff performance, attendance, discipline, and lesson plan progress.": "جاري جلب أداء الموظفين والحضور والانضباط وتقدم خطط الدروس.",
  "Fees Management": "إدارة الرسوم",
  "Finance & Administration": "المالية والإدارة",
  "Fully Compliant": "متوافق بالكامل",

  // Navigation sections & items
  "Executive & Governance": "الإدارة التنفيذية والحوكمة",
  "Academic Control": "الرقابة الأكاديمية",
  "Theology & IPLE": "اللاهوت و IPLE",
  "Finance & Accounts": "المالية والحسابات",
  "Campus Operations": "عمليات الحرم المدرسي",
  "Residential Life": "الحياة السكنية",
  "Systems & HR": "الأنظمة والموارد البشرية",
  "Personal Workspace": "مساحة العمل الشخصية",

  "Director Users": "مستخدمو المدير",
  "Approval Workflow": "سير عمل الموافقات",
  "Headteacher Hub": "مركز ناظر المدرسة",
  "Manager Command": "مركز المدير",
  "Staff Performance": "أداء الموظفين",
  "Governance Board": "مجلس الحوكمة",
  "Ministry & Compliance": "الوزارة والامتثال",
  "Manager Approvals": "موافقات المدير",
  "DOS Command Center": "مركز قيادة مدير الدراسات",
  "Theology DOS Hub": "مركز مدير دراسات اللاهوت",
  "IPLE Management": "إدارة IPLE",
  "Subjects": "المواد الدراسية",
  "Learner Management": "إدارة المتعلمين",
  "Staff & Teacher Hub": "مركز الموظفين والمعلمين",
  "Academic Oversight": "الإشراف الأكاديمي",
  "Active Staff": "الموظفون النشطون",
  "Agenda items set": "تم تحديد بنود جدول الأعمال",
  "Approvals & Compliance": "الموافقات والامتثال",
  "Audit: Satisfactory": "التدقيق: مرضٍ",
  "Academic Classes": "الفصول الأكاديمية",
  "Madrasa & Islamic": "المدرسة الإسلامية",
  "Digital Homework": "الواجبات الرقمية",
  "Enter Marks": "إدخال الدرجات",
  "School Timetables": "الجداول المدرسية",
  "Curriculum Coverage": "تغطية المنهج",
  "Exams & Grading": "الامتحانات والتقييم",
  "Marks & Reports": "الدرجات والتقارير",
  "Learner Attendance": "حضور المتعلمين",
  "P7 Management": "إدارة P7",
  "Lesson Tracking": "تتبع الدروس",
  "Curriculum Setup": "إعداد المنهج",
  "Scheme of Work": "خطة العمل",
  "Syllabus Reports": "تقارير المنهج",
  "My Theology Dashboard": "لوحة تحكم اللاهوت",
  "Islamic Marks Entry": "إدخال الدرجات الإسلامية",
  "Madrasa & Quran": "المدرسة والقرآن",
  "Oral Assessments": "التقييمات الشفوية",
  "Islamic Learners": "المتعلمون الإسلاميون",
  "Items awaiting your decision": "عناصر تنتظر قرارك",
  "Lesson Register": "سجل الدروس",
  "Lesson Planner": "مخطط الدروس",
  "Finance Overview": "نظرة مالية",
  "Financial Accounts": "الحسابات المالية",
  "Fees & Collections": "الرسوم والتحصيلات",
  "Procurement & Store": "المشتريات والمخزن",
  "Payroll & Salaries": "الرواتب والأجور",
  "HR & Payroll": "الموارد البشرية والرواتب",
  "Budget Planning": "تخطيط الميزانية",
  "Petty Cash Management": "إدارة النثرية",
  "Fees Tracking": "تتبع الرسوم",
  "Reconciliation": "التسوية",
  "Expense Approvals": "موافقات المصروفات",
  "Tax Reports": "تقارير الضرائب",
  "Finance Audit Log": "سجل تدقيق المالية",
  "Store & Inventory Hub": "مركز المخزن والمخزون",
  "Resource Inventory": "مخزون الموارد",
  "Receiving & Internal": "الاستلام والداخلي",
  "Stock Alerts": "تنبيهات المخزون",
  "Supplier Database": "قاعدة بيانات الموردين",
  "Gate Control Hub": "مركز التحكم في البوابة",
  "Vehicle Log": "سجل المركبات",
  "Exit Passes": "تصاريح الخروج",
  "Gate Handover": "تسليم البوابة",
  "Office Management": "إدارة المكتب",
  "Official Documents": "الوثائق الرسمية",
  "Print Orders": "طلبات الطباعة",
  "Delivery Log": "سجل التوصيل",
  "Comms & Circulars": "الاتصالات والتعاميم",
  "Medical Infirmary Hub": "مركز العيادة الطبية",
  "Prescriptions": "الوصفات الطبية",
  "Pharmacy Inventory": "مخزون الصيدلية",
  "Pharmacy Reports": "تقارير الصيدلية",
  "Health Records": "السجلات الصحية",
  "Residential Hostels": "المهاجع السكنية",
  "Orphanage Services": "خدمات الأيتام",
  "Discipline & Conduct": "الانضباط والسلوك",
  "Library": "المكتبة",
  "Kitchen Operations": "عمليات المطبخ",
  "Store Orders": "طلبات المخزن",
  "Food Inventory": "مخزون الطعام",
  "Meal Log": "سجل الوجبات",
  "Weekly Menu": "القائمة الأسبوعية",
  "Matron Dashboard": "لوحة تحكم المشرفة",
  "Dormitories": "المهاجع",
  "Residents": "المقيمون",
  "Essentials": "المستلزمات الأساسية",
  "Supplies": "الإمدادات",
  "Washing & Laundry": "الغسيل والتنظيف",
  "Holiday Arrivals": "وصول العطلات",
  "HR Management": "إدارة الموارد البشرية",
  "Staff Daily Attendance": "حضور الموظفين اليومي",
  "Asset & Item Tracking": "تتبع الأصول والأصناف",
  "ID Card System": "نظام بطاقات الهوية",
  "Access Control": "التحكم في الوصول",
  "Compliance Portal": "بوابة الامتثال",
  "My Dashboard": "لوحة التحكم",
  "Staff Messaging": "رسائل الموظفين",
  "My Managed Classes": "فصولي المدارة",
  "Learner Gradebook": "سجل درجات المتعلمين",
  "Parent Communication": "التواصل مع أولياء الأمور",
  "Staff Letters": "خطابات الموظفين",
  "My Attendance": "حضوري",
  "Next Session": "الجلسة القادمة",
  "No documents uploaded.": "لم يتم رفع أي مستندات.",
  "No items pending": "لا توجد عناصر معلقة",
  "No scheduled sessions": "لا توجد جلسات مجدولة",
  "My Leave Requests": "طلبات الإجازة",
  "Events Calendar": "تقويم الأحداث",
  "Personal Finance": "المالية الشخصية",
  "Attendance Device": "جهاز الحضور",

  // Page titles
  "Policies Active": "السياسات النشطة",
  "Policy Library": "مكتبة السياسات",
  "Primary decision-making body and executive committee": "الهيئة الرئيسية لاتخاذ القرارات واللجنة التنفيذية",
  "Homework": "الواجبات",
  "Hostel": "المهجع",
  "Budget": "الميزانية",
  "School Management Board (SMB)": "مجلس إدارة المدرسة (SMB)",
  "Staff Administration": "إدارة الموظفين",
  "Strategic dashboards for the center director": "لوحات معلومات استراتيجية لمدير المركز",
  "Strategic Oversight, Policy Framework & SMB Operations": "الإشراف الاستراتيجي، إطار السياسات وعمليات مجلس إدارة المدرسة",
  "Strategic Score": "الدرجة الاستراتيجية",
  "Student Welfare & Conduct": "رعاية الطلاب والسلوك",
  "Top & bottom performers": "الأفضل والأقل أداءً",
  "View All Frameworks": "عرض جميع الأطر",

  // Auth / Login
  "Email address": "البريد الإلكتروني",
  "Forgot password?": "نسيت كلمة المرور؟",

  // Common actions
  "Save Changes": "حفظ التغييرات",
  "Are you sure?": "هل أنت متأكد؟",
  "This action cannot be undone.": "لا يمكن التراجع عن هذا الإجراء.",
  "Continue": "متابعة",
  "Back": "رجوع",
  "Confirm": "تأكيد",
  "Yes": "نعم",
  "No": "لا",
  "OK": "حسناً",
  "Learn more": "اعرف المزيد",
  "Show more": "عرض المزيد",
  "Show less": "عرض أقل",
  "View all": "عرض الكل",
  "More info": "مزيد من المعلومات",
  "No results": "لا توجد نتائج",
  "No items": "لا توجد عناصر",
  "Type here...": "اكتب هنا...",
  "Select...": "اختر...",
  "Choose...": "اختر...",
  "Loading": "جاري التحميل",
  "Saving": "جاري الحفظ",
  "Processing": "جاري المعالجة",
  "Done": "تم",
  "Updated": "تم التحديث",
  "Created": "تم الإنشاء",
  "Removed": "تم الإزالة",
  "Canceled": "ملغي",
  "On Hold": "معلق",
  "Open Issues": "القضايا المفتوحة",
  "Open queue": "فتح قائمة الانتظار",
  "Open scorecards": "فتح بطاقات الأداء",
  "In Progress": "قيد التنفيذ",
  "Awaiting": "قيد الانتظار",
  "Approved": "تمت الموافقة",
  "Rejected": "مرفوض",
  "Submitted": "تم الإرسال",

  // Page Tour
  "Page Tour": "جولة في الصفحة",
  "Welcome to Alheib": "مرحباً بك في الحبيب",
  "Alheib Command Center": "مركز قيادة الحبيب",
  "Smart Navigation": "التنقل الذكي",
  "Operational Pulse": "نبض العمليات",
  "Financial Overview": "نظرة مالية",
  "Quick Action Center": "مركز الإجراءات السريعة",
  "Theology Teacher Dashboard": "لوحة تحكم معلم اللاهوت",
  "Quick Access Links": "روابط الوصول السريع",
  "IPLE Core Subjects": "المواد الأساسية لـ IPLE",

  // Empty states
  "No data": "لا توجد بيانات",
  "Nothing here yet": "لا يوجد شيء هنا بعد",
  "Get started": "ابدأ الآن",
  "Start by adding": "ابدأ بإضافة",
  "Start by creating": "ابدأ بإنشاء",

  // Weekdays
  "Monday": "الإثنين",
  "Tuesday": "الثلاثاء",
  "Wednesday": "الأربعاء",
  "Thursday": "الخميس",
  "Friday": "الجمعة",
  "Saturday": "السبت",
  "Sunday": "الأحد",
  "Mon": "إثن",
  "Tue": "ثلاث",
  "Wed": "أربع",
  "Thu": "خميس",
  "Fri": "جمعة",
  "Sat": "سبت",
  "Sun": "أحد",

  // Manager Performance
  "Approvals Queue": "قائمة الموافقات",
  "Supervisor / Head of Department sign-off": "اعتماد المشرف / رئيس القسم",
  "Checked in successfully": "تم تسجيل الحضور بنجاح",
  "Failed to check in": "فشل تسجيل الحضور",
  "Unable to load performance data.": "تعذر تحميل بيانات الأداء.",
  "Please refresh to retry.": "يرجى التحديث لإعادة المحاولة.",
  "Manager scorecard across teaching and operations": "بطاقة أداء المدير عبر التدريس والعمليات",
  "Checking in...": "جاري تسجيل الحضور...",
  "Self Check-In": "تسجيل حضور ذاتي",
  "Quarterly Review Export": "تصدير المراجعة الربعية",
  "Avg. Attendance": "متوسط الحضور",
  "+2.1% from last month": "+2.1% عن الشهر الماضي",
  "On track for Term 2": "ضمن الخطة للفصل الثاني",
  "Parent Feedback": "تقييم أولياء الأمور",
  "Based on live feedback metrics": "بناءً على مقاييس التقييم المباشرة",
  "Cases Handled": "الحالات التي تم التعامل معها",
  "Live disciplinary case count": "عدد حالات الانضباط المباشرة",
  "Performance Overview": "نظرة عامة على الأداء",
  "Aggregate metrics comparison": "مقارنة المقاييس الإجمالية",
  "Staff Scorecards": "بطاقات أداء الموظفين",
  "Detailed individual metrics": "مقاييس فردية مفصلة",
  "All roles": "جميع الأدوار",
  "Ops Reliability": "موثوقية العمليات",
  "No staff members found matching your filters": "لم يتم العثور على موظفين يطابقون عوامل التصفية",
  "Attendance Reliability Trend": "اتجاه موثوقية الحضور",
  "Monthly average across all departments": "المتوسط الشهري عبر جميع الأقسام",
  "Syllabus Completion by Grade": "إكمال المنهج حسب الصف",
  "Estimated progress against curriculum": "التقدم المقدر وفق المنهج الدراسي",
  "N/A": "غ.م",
  "cases": "حالات",

  // Months
  "January": "يناير",
  "February": "فبراير",
  "March": "مارس",
  "April": "أبريل",
  "May": "مايو",
  "June": "يونيو",
  "July": "يوليو",
  "August": "أغسطس",
  "September": "سبتمبر",
  "October": "أكتوبر",
  "November": "نوفمبر",
  "December": "ديسمبر",
};

// Seed the async cache with all dictionary entries
for (const [k, v] of Object.entries(enToAr)) {
  translationCache.set(k, v);
}

// Convert ASCII digits to Arabic-Indic digits (٠١٢٣٤٥٦٧٨٩) — used in Arabic mode.
const arDigits = "٠١٢٣٤٥٦٧٨٩";
export const toArabicDigits = (input: string): string =>
  input.replace(/[0-9]/g, (d) => arDigits[Number(d)]);

// Translate any plain string. Only does exact full-string matches (no partial token replacement
// that would produce embarrassing mixed Arabic/English text). Unknown strings stay English
// until the async Ollama/LibreTranslate pass translates them in full.
// Also checks translationCache so previously-async-translated strings resolve instantly
// on subsequent React re-renders.
export const translateText = (raw: string, lang: "en" | "ar"): string => {
  if (lang !== "ar") return raw;
  const trimmed = raw.trim();
  if (!trimmed) return raw;
  // Don't attempt to translate URLs, markdown links, or code-like tokens
  if (/https?:\/\//.test(raw) || /\[[^\]]+\]\([^\)]+\)/.test(raw) || /^[_A-Za-z0-9\-]+$/.test(trimmed)) return raw;
  // If already Arabic, just convert digits
  if (/[\u0600-\u06FF]/.test(raw) && !/[A-Za-z]/.test(raw)) return toArabicDigits(raw);
  // Exact full-string dictionary match only
  const direct = enToAr[trimmed];
  if (direct) return raw.replace(trimmed, toArabicDigits(direct));
  // Check async cache (populated by previous Ollama/LibreTranslate passes)
  const cached = translationCache.get(trimmed);
  if (cached) return raw.replace(trimmed, toArabicDigits(cached));
  // If mixed Arabic/English, leave as-is (async pass will translate the whole string)
  return raw;
};
