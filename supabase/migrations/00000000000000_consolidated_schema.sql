...
ALTER PUBLICATION supabase_realtime ADD TABLE public.learners;-- Create app_role enum for different user types (includes all roles ever needed)
CREATE TYPE public.app_role AS ENUM ('admin', 'teacher', 'parent', 'staff',
  'head_teacher', 'accountant', 'security', 'direct_manager', 'center_director',
  'office_manager', 'storekeeper', 'gateman', 'nurse', 'dos', 'director',
  'deputy_head_teacher', 'secretary', 'head_of_internal', 'dos_theology',
  'theology_teacher', 'orphan_supervisor', 'matron', 'cook', 'store_manager', 'manager');
...
-- Staff members are added via auth.users trigger (no hardcoded seed data)
...
-- Ensure learner columns exist before view references them
ALTER TABLE public.learners ADD COLUMN IF NOT EXISTS parent_name TEXT;
ALTER TABLE public.learners ADD COLUMN IF NOT EXISTS parent_phone TEXT;
ALTER TABLE public.learners ADD COLUMN IF NOT EXISTS parent_nin TEXT;
ALTER TABLE public.learners ADD COLUMN IF NOT EXISTS special_needs_category TEXT;
...
-- Add school_id to learners for EMIS view
ALTER TABLE public.learners ADD COLUMN IF NOT EXISTS school_id UUID REFERENCES public.schools(id);
...
-- Add unique constraint to profile_id before the upsert
DO $$
BEGIN
...
-- 9. SAMPLE TEST ACCOUNTS (skipped for fresh project)
-- Sample data insertion omitted to avoid enum/transaction issues.
-- Use Supabase UI or SQL Editor to create test users manually.
...
BEGIN
    FOREACH t IN ARRAY table_list LOOP
        ...
        EXCEPTION WHEN undefined_table THEN
            NULL; -- skip tables not yet created
        END;
    END LOOP;
END $$;
...
-- Add columns for parent_learner_links needed by the view
ALTER TABLE public.parent_learner_links ADD COLUMN IF NOT EXISTS parent_id UUID REFERENCES public.guardians(id);
ALTER TABLE public.parent_learner_links ADD COLUMN IF NOT EXISTS is_primary_contact BOOLEAN DEFAULT false;

-- View for Parent Chat compatibility
DROP VIEW IF EXISTS public.learner_parents CASCADE;
...
-- Ensure teacher_id column exists for the INSERT below
ALTER TABLE public.academic_warnings ADD COLUMN IF NOT EXISTS teacher_id UUID REFERENCES public.profiles(id);
ALTER TABLE public.academic_warnings ADD COLUMN IF NOT EXISTS category TEXT;
ALTER TABLE public.academic_warnings ADD COLUMN IF NOT EXISTS details TEXT;
...
-- Add status column to exam_series for the sample data below
ALTER TABLE public.exam_series ADD COLUMN IF NOT EXISTS status TEXT DEFAULT 'scheduled';
...
-- Sample Curriculum Data (skipped for fresh project - no data exists)
-- Use Supabase UI or SQL Editor to seed sample curriculum data manually.
...
-- Learners are added by school admin via the application (no hardcoded seed data)
